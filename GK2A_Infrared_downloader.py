...
# ==================== 核心配置（仅红外图片） ====================
SAVE_BASE_DIR = "/DATA/Gallery/GK-2A/Infrared"  # 红外图片保存目录
LOG_DIR = "/DATA/logs/GK2A_Infrared"  # 独立日志目录
DOWNLOAD_HISTORY_FILE = os.path.join(LOG_DIR, "infrared_download_history.json")  # 独立历史记录

# 红外图片专属配置（修正URL模板，确保有效性）
CONFIG = {
    "name": "GK2A_Infrared",
    "url_template": "https://nmsc.kma.go.kr/IMG/GK2A/AMI/PRIMARY/L1B/COMPLETE/FD/{ym}/{dd}/{hh}/gk2a_ami_le1b_ir105_fd020ge_{ymd_hh_nn}.srv.png",
    "publish_mins": [0, 20, 40],   # 每小时0/20/40分发布（与彩色错开）
    "download_delay_mins": 20,      # 发布后20分钟下载
    "min_file_size": 1024 * 100,   # 最小100KB（低于此视为错误文件）
...
def setup_logger():
    ...
    log_file = os.path.join(LOG_DIR, "gk2a_infrared_download.log")
...
def create_required_dirs():
    ...
    try:
        ...
        logger.info(f"红外图片保存目录准备就绪：{SAVE_BASE_DIR}（有效文件永久保留，不自动删除）")
...
def load_download_history() -> Dict[str, Any]:
    ...
    if not os.path.exists(DOWNLOAD_HISTORY_FILE):
        logger.info("红外图片下载历史文件不存在，使用默认配置（补下载最近1天）")
    ...
    try:
        ...
        logger.info(f"成功加载红外图片下载历史，已记录 {len(history['downloaded'])} 个下载任务（文件均保留）")
        return history
    except Exception as e:
        logger.error(f"加载红外图片下载历史失败，使用默认配置：{str(e)}")
        save_download_history(default_history)
        return default_history
...
def add_download_history(pt_str: str, history: Dict[str, Any]):
    ...
    if pt_str not in history["downloaded"]:
        ...
        logger.info(f"历史记录清理：保留最近30天任务记录，实际文件仍永久保留在 {SAVE_BASE_DIR}")
...
def generate_missing_publish_times(history: Dict[str, Any]) -> List[datetime.datetime]:
    ...
    try:
        ...
        logger.warning("红外图片 - 最后下载时间格式错误，使用1天前作为起始时间")
    
    current_dt = datetime.datetime.now(datetime.UTC)
    start_dt = max(last_dt, current_dt - datetime.timedelta(days=MAX_RECOVER_DAYS))
    
    logger.info(f"红外图片 - 补下载时间范围：{start_dt.strftime('%Y-%m-%d %H:%M:%S')} UTC 至 {current_dt.strftime('%Y-%m-%d %H:%M:%S')} UTC")
    ...
    logger.info(f"红外图片 - 共发现 {len(missing_times)} 个未下载的发布时间点")
...
def is_file_valid(save_path: str) -> bool:
    """验证红外图片文件有效性（仅删除错误文件，有效文件绝不删除）"""
    if not os.path.exists(save_path):
        return False
    
    file_size = os.path.getsize(save_path)
    # 仅删除大小不达标的错误文件（有效文件即使历史记录清理也不删）
    if file_size < CONFIG["min_file_size"]:
        logger.warning(f"红外图片 - 文件不完整（{file_size}字节 < {CONFIG['min_file_size']}字节），属于错误文件：{save_path}")
        if os.path.exists(save_path):
            os.remove(save_path)
            logger.info(f"红外图片 - 已删除不完整的错误文件：{save_path}（仅错误文件会被删除）")
        return False
    ...
    with open(save_path, "rb") as f:
        ...
        if magic != CONFIG["file_magic"]:
            logger.warning(f"红外图片 - 无效PNG文件（文件头：{magic.hex()}），属于错误文件：{save_path}")
            os.remove(save_path)
            logger.info(f"红外图片 - 已删除非PNG格式的错误文件：{save_path}（仅错误文件会被删除）")
            return False
    
    logger.info(f"红外图片 - 文件验证有效：{save_path}（将永久保留，不自动删除）")
    return True
...
def download_file(url: str, save_path: str, pt: datetime.datetime, pt_str: str, history: Dict[str, Any], is_recover: bool = False) -> bool:
    """下载红外图片（仅删除临时文件和错误文件，有效文件永久保留）"""
    prefix = "[补下载]" if is_recover else ""
    logger.info(f"红外图片 {prefix} - 开始下载：{url}")
    logger.info(f"红外图片 {prefix} - 保存路径：{save_path}（有效文件将永久保留）")
    if is_recover:
        logger.info(f"红外图片 {prefix} - 发布时间：{pt_str} UTC")

    # 若文件已存在且有效，直接返回成功（不重复下载，不删除）
    if os.path.exists(save_path) and is_file_valid(save_path):
        logger.info(f"红外图片 {prefix} - 文件已存在且有效，跳过下载：{save_path}（永久保留）")
        add_download_history(pt_str, history)
        return True
    ...
    for retry in range(MAX_RETRIES):
        try:
            ...
            if response.status_code != 200:
                logger.error(f"红外图片 {prefix} - 下载失败（HTTP {response.status_code}）：{url}")
                continue
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if (current_time - last_progress_time >= DOWNLOAD_PROGRESS_INTERVAL) or (downloaded_size == total_size):
                            ...
                            logger.info(f"红外图片 {prefix} - 下载进度：{progress_bar}")
                            last_progress_time = current_time

            if is_file_valid(save_path):
                current_utc = datetime.datetime.now(datetime.UTC)
                logger.info(f"红外图片 {prefix} - 下载成功：{save_path}（有效文件，永久保留，绝不自动删除）")
                logger.info(f"红外图片 {prefix} - 当前UTC时间：{current_utc.strftime('%Y-%m-%d %H:%M:%S')}")
                logger.info(f"红外图片 {prefix} - " + "=" * 50)
                add_download_history(pt_str, history)
                return True
            else:
                logger.warning(f"红外图片 {prefix} - 下载的文件为错误文件：{save_path}")
        
        except KeyboardInterrupt:
            logger.info(f"\n红外图片 {prefix} - 下载被手动中断，清理未完成的临时文件...")
            # 仅删除未下载完成的临时文件（大小不达标），已完成的有效文件绝不删除
            if os.path.exists(save_path) and os.path.getsize(save_path) < CONFIG["min_file_size"]:
                os.remove(save_path)
                logger.info(f"红外图片 {prefix} - 已删除未完成的临时文件：{save_path}（仅临时文件会被删除）")
            raise
        
        except RequestException as e:
            logger.error(f"红外图片 {prefix} - 下载失败（重试 {retry+1}/{MAX_RETRIES}）：{str(e)}")
            if retry < MAX_RETRIES - 1:
                time.sleep(RETRY_DELAY + retry * 2)
        except Exception as e:
            logger.error(f"红外图片 {prefix} - 下载异常（重试 {retry+1}/{MAX_RETRIES}）：{str(e)}", exc_info=True)
            if retry < MAX_RETRIES - 1:
                time.sleep(RETRY_DELAY + retry * 2)
        finally:
            if response:
    ...
    logger.error(f"红外图片 {prefix} - 所有重试失败：{url}")
    return False
...
def execute_recover_download(history: Dict[str, Any]):
    """执行红外图片补下载（仅下载缺失文件，不删除任何已有有效文件）"""
    global failed_tasks
    logger.info("=" * 80)
    logger.info("开始执行GK-2A红外图片补下载（仅下载缺失文件，不删除任何有效文件）")
    logger.info("=" * 80)
    ...
    try:
        ...
        for pt in missing_times:
            ...
            if is_downloaded(pt_str, history):
                logger.info(f"红外图片 - 已下载（文件保留），跳过：{pt_str} UTC")
                continue
            ...
            if download_file(url, save_path, pt, pt_str, history, is_recover=True):
                ...
                total_fail += 1
                logger.info(f"红外图片 - 补下载失败，10分钟后重试：{pt_str} UTC")
                logger.info(f"红外图片 - " + "=" * 50)
...
def main():
    ...
    logger.info("核心规则：已下载的有效文件绝不自动删除，仅清理错误文件和未完成的临时文件")
    logger.info("=" * 80)
        ...
        try:
            ...
            if current_timestamp >= next_progress_refresh:
                ...
                if next_window:
                    ...
                    logger.info(f"红外图片 | 距离下次下载窗口：{progress_bar} | 下次窗口：{next_window.strftime('%Y-%m-%d %H:%M:%S')} UTC")
                else:
                    logger.info(f"红外图片 | 无法计算下次下载窗口")
                ...
                for pt_str, first_fail_time in failed_tasks.items():
                    if current_timestamp - first_fail_time >= FAILED_RETRY_DELAY:
                        ...
                        if download_file(url, save_path, pt, pt_str, download_history):
                            logger.info(f"红外图片 - 重试成功：{pt_str} UTC（文件永久保留）")
                        else:
                            logger.error(f"红外图片 - 重试失败，跳过：{pt_str} UTC")
            ...
            if pt:
                ...
                if not download_file(url, save_path, pt, pt_str, download_history):
                    ...
                    logger.info(f"红外图片 - 首次下载失败，10分钟后重试：{pt_str} UTC")
                    logger.info(f"红外图片 - " + "=" * 50)
    ...
    logger.info("GK-2A红外图片下载脚本已退出（所有有效文件均永久保留）")
